...
if (isset($_POST['guestdetailsubmit'])) {
  ...
  $Email = $_SESSION['usermail']; // Always use session email!
  ...
  if ($Name && $Email && $Phone && $HouseType && $cin && $cout) {
    // Check overlap for the same house (any status except cancelled)
...
<head>
  ...
  <style>
    ...
    #guestdetailpanel .middle { height: 350px; }
    .mybookingstable { width: 92%; margin: 24px auto 48px auto; background: #fff; border-radius: 16px; box-shadow: 0 2px 8px #eee;}
      ...
      <form method="POST" class="guestdetailpanelform">
        ...
        <div class="middle">
          <div class="guestinfo">
            ...
            <input type="email" name="Email" placeholder="Email" required value="<?php echo $_SESSION['usermail']; ?>" readonly style="background:#eee">
...
<select name="HouseType" required>
  ...
  <?php
    ...
    while ($hr = mysqli_fetch_assoc($hq)) {
      $housename = htmlspecialchars($hr['HouseName']);
      echo "<option value=\"$housename\">$housename</option>";
    }
  ?>
</select>
...
<section id="secondsection">
  ...
  <div class="ourroom"><h1>≼ Our Homestays ≽</h1>
...
<script>
  ...
  document.querySelector(".guestdetailpanelform").addEventListener("submit", function(e) {
    const cin = new Date(this.cin.value);
    const cout = new Date(this.cout.value);
    if (cin >= cout) {
      e.preventDefault();
      swal({ title: "Check-in must be before Check-out!", icon: "error" });
      return false;
    }
  });

  // Prevent overlap (disable booked dates)
  let booked = [];
  <?php
  $q = mysqli_query($conn, "SELECT House, CheckIn, CheckOut FROM housebook WHERE Status != 'Cancelled'");
  while ($r = mysqli_fetch_assoc($q)) {
    echo "booked.push({ house:'{$r['House']}', start:'{$r['CheckIn']}', end:'{$r['CheckOut']}' });\n";
  }
  ?>
  ["HouseType","cin","cout"].forEach(name => document.querySelector(`[name='${name}']`).addEventListener("change", checkOverlap));
  function checkOverlap() {
    const house = document.querySelector("[name='HouseType']").value;
    const cin = document.querySelector("[name='cin']").value;
    const cout = document.querySelector("[name='cout']").value;
    if (!house || !cin || !cout) return;
    const clash = booked.some(r => r.house == house && new Date(r.start) < new Date(cout) && new Date(r.end) > new Date(cin));
    if (clash) {
      swal({ title: "These dates are booked for selected house!", icon: "error" });
      document.querySelector("[name='cin']").value = "";
      document.querySelector("[name='cout']").value = "";
    }
  }
</script>
